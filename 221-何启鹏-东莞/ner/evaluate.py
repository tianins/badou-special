...
class Evaluator:
    ...
    def eval(self, epoch):
        ...
        for index, batch_data in enumerate(self.valid_data):
            sentences = self.valid_data.dataset.sentences[index * self.config["batch_size"]: (index+1) * self.config["batch_size"]]  # 提取出测试集的64句话
    ...
    def write_stats(self, labels, pred_results, sentences):
        ...
        if not self.config["use_crf"]:
            pred_results = torch.argmax(pred_results, dim=-1)  # [64,150]从预测结果中取最大概率位置
        for true_label, pred_label, sentence in zip(labels, pred_results, sentences):
            if not self.config["use_crf"]:
                pred_label = pred_label.cpu().detach().tolist()  # 转换为list
